...
- `[x]` Migration: Tạo bảng board_members và thêm cột is_private cho boards
- `[x]` Migration: Tạo bảng checklists, comments, activities
- `[x]` Migration: Tạo bảng tags và card_tags
- `[x]` Tạo API quản lý Board Members (Add/Remove User khỏi Board)
- `[x]` Tạo API cập nhật chi tiết Card (Mô tả, Assignee, Due Date)
- `[x]` Tạo API Checklist (Thêm mục, Đánh dấu hoàn thành)
- `[x]` Tạo API quản lý Tags (Tạo mới, Gán Tag, Xóa Tag)
- `[x]` Tạo API Comment (Thêm bình luận)
- `[x]` Cấu hình Entity Listener ghi nhận Activity Log tự động
- `[x]` Tạo API quản lý User Profile (Cập nhật thông tin cá nhân)

### 12. Frontend - Card Detail, Board Members & Profile
- `[x]` Dựng giao diện quản lý Board Members (Danh sách Avatar và Nút Add trên Header)
...
- `[x]` Dựng giao diện quản lý Tags (Trong Modal và hiển thị trên Thẻ)
- `[x]` Dựng giao diện Comment và hiển thị Activity Log
- `[x]` Dựng trang User Profile (Cập nhật thông tin cá nhân, ảnh đại diện)
- `[x]` Thêm Biểu đồ thống kê gọn nhẹ trên Dashboard
  - `[x]` Tạo DashboardStatsDTO
  - `[x]` Cập nhật các Repository (Workspace, Board, Card)
  - `[x]` Tạo DashboardService và DashboardController
  - `[x]` Cài đặt thư viện recharts trên Frontend
  - `[x]` Cập nhật DashboardOverview.tsx để hiển thị số liệu và biểu đồ
...
- `[x]` Frontend: Dựng UI chọn Assignee (Popover/Dropdown) trong Card Detail Modal
- `[x]` Frontend: Hiển thị Avatar của Assignee ngay trên Thẻ (Card) ở màn hình Board

### 14.5. Gán Thành viên vào Checklist (Checklist Assignees)
- `[x]` Database: Tạo file Migration V10 thêm `assignee_id`
- `[x]` Backend: Cập nhật Entity, DTO, ChecklistService
