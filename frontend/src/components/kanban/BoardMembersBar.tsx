...
const BoardMembersBar: React.FC<BoardMembersBarProps> = ({ boardId }) => {
    ...
    const updateRoleMutation = useMutation({
        mutationFn: ({ userId, role }: { userId: number, role: string }) => updateBoardMemberRole(boardId, userId, role),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['boardMembers', boardId] });
        }
    });
    ...
    const currentMemberRole = members.find(m => m.email === currentUser.email)?.role || 'MEMBER';
    const isAdmin = currentMemberRole === 'ADMIN' || currentMemberRole === 'ROLE_ADMIN';
                    ...
                    <div className="py-2 max-h-48 overflow-y-auto custom-scrollbar">
                        {members.length === 0 ? (
                            ...
                            members.map((member) => (
                                <div
                                    key={member.id}
                                    className="flex items-center justify-between px-5 py-2 hover:bg-slate-50 group transition-colors"
                                >
                                    ...
                                    <div className="flex items-center space-x-2">
                                        ...
                                        {isAdmin && member.email !== currentUser.email ? (
                                            <select
                                                ...
                                                className="text-xs bg-white border border-slate-200 rounded-md py-1 px-2 focus:outline-none focus:ring-1 focus:ring-emerald-500 text-slate-600"
                                            >
                                            ...
                                            <span className="text-xs px-2 py-1 bg-slate-100 text-slate-500 rounded-md font-medium">
                                                {member.role === 'ADMIN' || member.role === 'ROLE_ADMIN' ? 'Admin' : (member.role === 'VIEWER' ? 'Viewer' : 'Member')}
                                            </span>
                                        )}
                                    </div>
                                </div>
